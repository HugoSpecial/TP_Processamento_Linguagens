...
def execute_command(parsed):
    ...
    if cmd_type == 'IMPORT' and len(parsed) >= 3:
        ...
        table_name, source_table, columns, conditions = parsed[1], parsed[2], parsed[3], parsed[4]
        
        # Tentar obter a tabela de origem
        source_table_data = database.get_table(source_table)
        
        if not source_table_data:
            print(f"[ERRO] Tabela '{source_table}' não encontrada.")
            return
        
        # Buscar dados e cabeçalhos da tabela de origem
        source_data = source_table_data.get('dados', [])
        headers = source_table_data.get('variaveis', [])
        
        # Verificar se os dados e cabeçalhos são válidos
        if not source_data or not headers:
            print(f"[ERRO] Dados ou cabeçalhos inválidos para a tabela '{source_table}'.")
            return
        
        # Filtrar dados com base nas condições (se houver)
        filtered_data = []
        ...
        # Se * foi usado, incluir todas as colunas
        if columns == '*':
            columns = headers  # Defina as colunas como todas as colunas da tabela de origem
        else:
            # Caso contrário, garantir que as colunas especificadas existam na tabela de origem
            if not all(col in headers for col in columns):
                print(f"[ERRO] Colunas {columns} não encontradas na tabela '{source_table}'.")
                return
        
        # Armazenar o resultado no novo nome de tabela
        if isinstance(columns, list):
            # Filtrar os dados para incluir apenas as colunas solicitadas
            filtered_columns_data = [
                [row[headers.index(col)] for col in columns] for row in filtered_data
            ]
            database.add_table(table_name, columns, filtered_columns_data)
            print(f"Tabela '{table_name}' criada com sucesso!")
        else:
            print("[ERRO] Colunas inválidas para a criação da tabela.")
            return
